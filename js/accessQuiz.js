...
const optionsContainer = document.querySelector(
  ".access-question--container .access-question--bottom .access-buttons div"
);
const error = document.querySelector(".error-container");
const nextBtn = document.querySelector(
  ".access-question--container .access-question--bottom .access-buttons .button-submit"
);
...
async function displayHTMLQuiz(index) {
  ...
  const data = results.quizzes[3].questions;

  //   Question Count
  const questionCountContent = document.querySelector(
    ".access-question--container .access-question-text .question-count"
  );
  ...
  //   Question
  const accessQuestionText = document.querySelector(
    ".access-question--container .access-question-text h2"
  );
  const questionTag = ` <span>${data[index].question}</span>`;
  accessQuestionText.innerHTML = questionTag;

  //   Options

  const optiontags =
    '<button class="btn btn-access"><div><span>A</span><div class="choice">' +
    escapeHTML(data[index].options[0]) +
    "</div></div><div></div></button>" +
    '<button class="btn btn-access"><div><span>B</span><div class="choice">' +
    escapeHTML(data[index].options[1]) +
    "</div></div><div></div></button>" +
    '<button class="btn btn-access"><div><span>C</span><div class="choice">' +
    escapeHTML(data[index].options[2]) +
    "</div></div><div></div></button>" +
    '<button class="btn btn-access"><div><span>D</span> <div class="choice">' +
    escapeHTML(data[index].options[3]) +
    "</div></div><div></div></button>";
  optionsContainer.innerHTML = optiontags;

  const option = optionsContainer.querySelectorAll(".btn-access");

  for (let i = 0; i < option.length; i++) {
    if (w.matches) {
      if (option[i].children[0].children[1].textContent.length > 45) {
    ...
    if (z.matches) {
      if (option[i].children[0].children[1].textContent.length > 15) {
        option[i].children[0].children[1].style.fontSize = "1.6rem";
      } else if (option[i].children[0].children[1].textContent.length > 25) {
        option[i].children[0].children[1].style.fontSize = "1.4rem";
      } else {
        option[i].children[0].children[1].style.fontSize = "1.8rem";
      }
    }
    ...
    if (userAns === correctAnser) {
      ...
      for (let i = 0; i < allOptions; i++) {
        ...
        if (optionText.textContent === correctAnser) {
          optionsContainer.children[i].setAttribute(
            "class",
            "btn-access correct"
          );
