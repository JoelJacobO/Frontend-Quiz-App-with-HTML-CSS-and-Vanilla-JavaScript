...
const optionsContainer = document.querySelector(
  ".js-question--container .access-question--bottom .access-buttons div"
);
const error = document.querySelector(".error-container");
const nextBtn = document.querySelector(
  ".js-question--container .access-question--bottom .access-buttons .button-submit"
);
...
async function displayHTMLQuiz(index) {
  ...
  const data = results.quizzes[2].questions;

  //   Question Count
  const questionCountContent = document.querySelector(
    ".js-question--container .js-question-text .question-count"
  );
  ...
  const cssQuestionText = document.querySelector(
    ".js-question--container .js-question-text h2"
  );
  ...
  const optiontags =
    '<button class="btn btn-js"><div><span>A</span><div class="choice">' +
    escapeHTML(data[index].options[0]) +
    "</div></div><div></div></button>" +
    '<button class="btn btn-js"><div><span>B</span><div class="choice">' +
    escapeHTML(data[index].options[1]) +
    "</div></div><div></div></button>" +
    '<button class="btn btn-js"><div><span>C</span><div class="choice">' +
    escapeHTML(data[index].options[2]) +
    "</div></div><div></div></button>" +
    '<button class="btn btn-js"><div><span>D</span> <div class="choice">' +
    escapeHTML(data[index].options[3]) +
    "</div></div><div></div></button>";
  optionsContainer.innerHTML = optiontags;

  const option = optionsContainer.querySelectorAll(".btn-js");
    ...
    if (userAns === correctAnser) {
      ...
      for (let i = 0; i < allOptions; i++) {
        ...
        if (optionText.textContent === correctAnser) {
          optionsContainer.children[i].setAttribute("class", "btn-js correct");
